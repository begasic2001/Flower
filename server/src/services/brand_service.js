...
const getAny = ({ page, limit, order, name, available, ...query }) => new Promise(async (resolve, reject) => {
  try {
    ...
    try {
      if (fileData) data.image = fileData?.path
        ...
        resolve({
            ...
            mes: response[0] > 0 ? `${response[0]} brand updated` : 'Cannot update new brand/ brand ID not found',
        })
...
const deleteBrand = (id, filename) => new Promise(async (resolve, reject) => {
  try {
